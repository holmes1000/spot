...
MonocularMode::MonocularMode() :Node("mono_node_cpp")
{
    ...
    // Create a publisher for the pose estimate
    pose_publisher_ = this->create_publisher<geometry_msgs::msg::PoseStamped>("/mono_py_driver/pose", 100);
...
void MonocularMode::Img_callback(const sensor_msgs::msg::Image& msg)
{
    ...
    pose_msg.pose.orientation.w = Tcw.unit_quaternion().w();
    pose_msg.pose.orientation.x = Tcw.unit_quaternion().x();
    pose_msg.pose.orientation.y = Tcw.unit_quaternion().y();
    pose_msg.pose.orientation.z = Tcw.unit_quaternion().z();
